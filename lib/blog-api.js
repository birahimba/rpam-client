  ...
  for (let page = 0; page < MAX_PAGES; page += 1) {
    ...
    if (!data.nextCursor) {
      if (articles.length === 0) {
        throw new Error(
          'Blog API : aucun article publié retourné. Le build est interrompu plutôt que de ' +
            'déployer un blog vide, ce qui supprimerait toutes les URLs /blog/<slug> des ' +
            "résultats de recherche. Vérifiez que les articles sont bien publiés dans le back-office."
        )
      }
